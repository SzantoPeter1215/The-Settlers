...
class PathFinder
{
    ...
    public static void main(String[] args)
    {
        ...
        if (path != null && path.size() > 0) {
            ...
            int[] XChords = new int[pathSize];
            int[] YChords = new int[pathSize];

            for(int i = 0; i < pathSize; ++i) {
                String[] raw = path.get(i).split(", ");
                XChords[i] = Integer.parseInt(String.valueOf(raw[0].charAt(1)));
                YChords[i] = Integer.parseInt(String.valueOf(raw[1].charAt(0)));
            }
            System.out.print("\n\n");
